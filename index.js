...
function createCategoryEmbed(category) {
    ...
    const categoryInfo = {
        'category_extra_owner': { name: 'Extra Owner System', icon: '🜲' },
        'category_quarantine': { name: 'Quarantine & Moderation', icon: '⊘' },
        'category_roles': { name: 'Role Management', icon: 'ⓘ' },
        'category_voice': { name: 'Voice Management', icon: '၊၊||၊' },
        'category_channels': { name: 'Channel Management', icon: '☰' },
        'category_media': { name: 'Media & Threads', icon: '🀦' },
        'category_automod': { name: 'Auto-Moderation', icon: '⛨' },
        'category_protection': { name: 'Bot & User Protection', icon: '𖢻' },
        'category_server': { name: 'Server Management', icon: '⛃' },
        'category_ticket': { name: 'Ticket System', icon: '✉︎' },
        'category_utility': { name: 'Utility Commands', icon: '𖠩' },
        'category_developer': { name: 'Developer Information', icon: '</>' }
    };
...
// Components v2 styled category embed - matches the Components v2 UI
function createCategoryEmbedV2(category, executor) {
    const totalCommands = getTotalCommandsCount();
    const separator = ackService.buildSeparator('white');
    ...
    const categoryContent = {
        ...
        'category_developer': {
            ...
            content: `**Total Commands:** ${totalCommands}\n${separator}\n` +
                ...
                `🌐 Website: https://scriptspace.in/\n` +
                `💬 Discord: discord.gg/scriptspace\n${separator}\n` +
                `**ᡣ𐭩 Protection Features**\n` +
                `✅ Anti-nuke protection\n` +
                `✅ Anti-raid detection\n` +
                `✅ Real-time monitoring\n${separator}\n` +
                `**📊 System Stats**\n` +
                `• **Response Time:** <1ms\n` +
                `• **Protection Level:** GOD-LEVEL\n` +
                `• **Servers:** ${client.guilds.cache.size}`
        }
    };
    
    const cardData = categoryContent[category] || { title: '✗ 𖹭 **Help Menu**', content: 'Select a category from the dropdown.' };
    
    const textDisplay = `**Time:** <t:${Math.floor(Date.now() / 1000)}:T>\n**Executed by:** <@${executor.id}>\n${separator}\n${cardData.title}\n${cardData.content}`;
    
    const embed = new EmbedBuilder()
        .setColor('#000000')
        .setDescription(textDisplay)
        .setThumbnail(ackService.imageURL)
        .setFooter({ text: `Category View • Components v2 UI`, iconURL: ackService.imageURL })
        .setTimestamp();
...
// Components v2 Help Card Content Builder
function getHelpCardContentV2(cardNumber) {
    const totalCommands = getTotalCommandsCount();
    const separator = ackService.buildSeparator('white');
    ...
    const cards = {
        1: {
            ...
            content: `**Total Commands:** ${totalCommands} • **Bot Owner Only**\n${separator}\n` +
                `**ᡣ𐭩 Extra Owner Management**\n` +
                `➤ \`extra owner @user\` - Grant permanent extra owner\n` +
                `➤ \`temp owner @user [duration]\` - Grant temporary owner\n` +
                `➤ \`remove owner @user\` - Remove extra owner status\n` +
                `➤ \`list owners\` - Show all extra owners\n${separator}\n` +
                `✿ **Durations:** 1h, 2h, 4h, 8h, 12h, 1d, 2d, 3d, 1w`
        },
        2: {
            title: '✗ 𖹭 **Quarantine & Basic Moderation**',
            content: `**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n${separator}\n` +
        ...
        3: {
            ...
            content: `**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n${separator}\n` +
                ...
                `➤ \`sethomevc #voice\` - Set home voice channel`
        },
        4: {
            title: '✗ 𖹭 **Advanced Role Management**',
            content: `**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n${separator}\n` +
                `**ᡣ𐭩 Advanced Role Commands**\n` +
                `➤ \`createrole <name> [color]\` / \`cr\` - Create role\n` +
                `➤ \`deleterole @role\` / \`dr\` - Delete role\n` +
                `➤ \`editrole @role <prop> <value>\` / \`er\` - Edit role\n` +
                `➤ \`roleinfo @role\` / \`ri\` - Show role info\n` +
                `➤ \`inrole @role\` - List members with role\n` +
                `➤ \`removeallroles @user\` / \`rar\` - Remove all roles`
        },
        5: {
            title: '✗ 𖹭 **Interim Role & Widget**',
            content: `**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n${separator}\n` +
                `**ᡣ𐭩 Interim Role Management**\n` +
                `➤ \`prmtr @user\` - Make interim role permanent\n` +
                `➤ \`revtr @user\` - Revoke interim role\n` +
                `➤ \`remtr @role\` - Remove from interim list\n` +
                `➤ \`addtr @role\` - Add to interim list\n${separator}\n` +
                `**ᡣ𐭩 Widget Commands**\n` +
                `➤ \`sendinterim\` - Send interim role widget\n` +
                `➤ \`intrch #channel\` - Set interim channel`
        },
        6: {
            title: '✗ 𖹭 **Voice Management**',
            content: `**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n${separator}\n` +
                `**ᡣ𐭩 Voice Mute Commands**\n` +
                `➤ \`vmute @user\` - Server mute user in VC\n` +
                `➤ \`vunmute @user\` - Server unmute user\n` +
                `➤ \`vmuteall\` - Mute all in your VC\n` +
                `➤ \`vunmuteall\` - Unmute all in your VC\n${separator}\n` +
                `**ᡣ𐭩 Voice Deafen Commands**\n` +
                `➤ \`vdefend @user\` - Server deafen user\n` +
                `➤ \`vundefend @user\` - Undeafen user\n` +
                `➤ \`vdefendall\` - Deafen all in VC`
        },
        7: {
            title: '✗ 𖹭 **Channel Management**',
            content: `**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n${separator}\n` +
                `**ᡣ𐭩 Text Channel Commands**\n` +
                `➤ \`lock\` / \`unlock\` - Lock/unlock channel\n` +
                `➤ \`hide\` / \`show\` - Hide/show channel\n` +
                `➤ \`slowmode <seconds>\` - Set slowmode\n${separator}\n` +
                `**ᡣ𐭩 Voice Channel Commands**\n` +
                `➤ \`lockvc\` / \`unlockvc\` - Lock/unlock VC\n` +
                `➤ \`hidevc\` / \`showvc\` - Hide/show VC\n` +
                `➤ \`limit <number>\` - Set user limit`
        },
        8: {
            title: '✗ 𖹭 **Security & Protection**',
            content: `**Total Commands:** ${totalCommands} • **Bot Owner Only**\n${separator}\n` +
                `**ᡣ𐭩 Security Commands**\n` +
                `➤ \`whitelist @user\` - Add to whitelist\n` +
                `➤ \`flagged\` - Show flagged users\n` +
                `➤ \`unflag @user\` - Remove from flaglist\n` +
                `➤ \`scanserver\` - Scan for threats\n${separator}\n` +
                `**ᡣ𐭩 Emergency Commands**\n` +
                `➤ \`panic\` / \`panicmode\` - Activate panic mode\n` +
                `➤ \`stop\` / \`end\` - Stop panic mode`
        },
        9: {
            title: '✗ 𖹭 **Utility Commands**',
            content: `**Total Commands:** ${totalCommands} • **All Channels**\n${separator}\n` +
        ...
        10: {
            title: '✗ 𖹭 **Say & Announcement**',
            content: `**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n${separator}\n` +
                `**ᡣ𐭩 Say Commands**\n` +
                `➤ \`say <message>\` - Send message as bot\n` +
                `➤ \`sayembed <message>\` - Send embedded message\n` +
                `➤ \`edit <msgId> <text>\` - Edit bot message\n` +
                `➤ \`reply <msgId> <text>\` - Reply to message\n${separator}\n` +
                `**ᡣ𐭩 Announcement Commands**\n` +
                `➤ \`annoc <text>\` - Server announcement\n` +
                `➤ \`annox <text>\` - Styled announcement`
        },
        11: {
            title: '✗ 𖹭 **Bot Information**',
            content: `**Total Commands:** ${totalCommands}\n${separator}\n` +
                `**ᡣ𐭩 Protection Features**\n` +
                `✅ Anti-nuke protection\n` +
                `✅ Anti-raid detection\n` +
    ...
    return cards[cardNumber] || cards[1];
}

// Build Components v2 Help Embed
function buildHelpEmbedV2(executor, cardNumber) {
    const cardData = getHelpCardContentV2(cardNumber);
    const separator = ackService.buildSeparator('white');
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(textDisplay)
        .setThumbnail(ackService.imageURL)
        .setFooter({ text: `Card ${cardNumber}/11 • Components v2 UI`, iconURL: ackService.imageURL })
        .setTimestamp();
    
    return embed;
}

// Function to create slideshow help with automatic card cycling - Components v2 UI
async function createHelpSlideshow(message, guildId) {
    try {
        ...
        // Build Components v2 UI help embed
        const helpEmbed = buildHelpEmbedV2(message.author, currentCard);
        
        // Add category dropdown
        const [categoryDropdown] = createCategoryDropdown(currentCard);
        ...
        const helpMessage = await message.reply({ 
            embeds: [helpEmbed], 
            components: [categoryDropdown, navRow] 
        });
        ...
        const slideshowInterval = setInterval(async () => {
            try {
                ...
                const updatedEmbed = buildHelpEmbedV2(message.author, currentCard);
                const [updatedDropdown] = createCategoryDropdown(currentCard);

                await helpMessage.edit({ 
                    embeds: [updatedEmbed], 
                    components: [updatedDropdown, navRow] 
                });
                console.log(`📋 Help slideshow (v2) updated to card ${currentCard} for ${message.author.username}`);
        ...
        // Auto-stop slideshow after 5 minutes
        setTimeout(() => {
            clearInterval(slideshowInterval);
            activeHelpSlideshows.delete(message.author.id);
            console.log(`📋 Help slideshow (v2) auto-stopped for ${message.author.username}`);
        }, 5 * 60 * 1000); // 5 minutes
        ...
        // Fallback to Components v2 single card
        const fallbackEmbed = buildHelpEmbedV2(message.author, 1);
        return await message.reply({ embeds: [fallbackEmbed] });
    }
}
...
client.on('interactionCreate', async interaction => {
    try {
        ...
        // Handle help category selection dropdown - Components v2 UI
        if (interaction.isStringSelectMenu() && interaction.customId === 'help_category_select') {
            ...
            try {
                ...
                // Use Components v2 styled embed
                const categoryEmbed = createCategoryEmbedV2(selectedCategory, interaction.user);
                const [categoryDropdown] = createCategoryDropdown();
                
                // Add navigation buttons with Components v2 styling
                const navRow = new ActionRowBuilder()
                    .addComponents(
                        new ButtonBuilder()
                            .setCustomId('help_prev')
                ...
                // Then edit the original message with Components v2 UI
                await interaction.editReply({
                    embeds: [categoryEmbed],
                    components: [categoryDropdown, navRow]
                });
                console.log(`📋 Category selected: ${selectedCategory} for ${interaction.user.username} (Components v2 UI)`);
            } catch (error) {
        ...
        // Handle help navigation buttons - Components v2 UI
        if (interaction.isButton() && (interaction.customId === 'help_prev' || interaction.customId === 'help_next' || interaction.customId === 'help_stop')) {
            try {
                // Get the current slideshow for this user
                const slideshow = activeHelpSlideshows.get(interaction.user.id);
                ...
                if (interaction.customId === 'help_stop') {
                    ...
                    const stoppedEmbed = new EmbedBuilder()
                        .setColor('#000000')
                        .setDescription(`**Time:** <t:${Math.floor(Date.now() / 1000)}:T>\n**Executed by:** <@${interaction.user.id}>\n${separator}\n✗ 𖹭 **Help Menu Stopped**\n\nThe help slideshow has been stopped.\nUse \`help\` command again to restart.`)
                        .setThumbnail(ackService.imageURL)
                        .setFooter({ text: 'Slideshow Stopped • Components v2 UI', iconURL: ackService.imageURL })
                        .setTimestamp();
                    ...
                    console.log(`📋 Help slideshow stopped by ${interaction.user.username}`);
                    return;
                }
                
                // Handle prev/next navigation
                // Extract current card number from the message footer
                const currentEmbed = interaction.message.embeds[0];
                let currentCard = 1;
                
                if (currentEmbed && currentEmbed.footer && currentEmbed.footer.text) {
                    const cardMatch = currentEmbed.footer.text.match(/Card (\d+)\/11/);
                    if (cardMatch) {
                        currentCard = parseInt(cardMatch[1]);
                    }
                }
                
                // Calculate new card number
                if (interaction.customId === 'help_prev') {
                    currentCard = currentCard <= 1 ? 11 : currentCard - 1;
                } else if (interaction.customId === 'help_next') {
                    currentCard = currentCard >= 11 ? 1 : currentCard + 1;
                }
                
                // Build new embed with Components v2 UI
                const updatedEmbed = buildHelpEmbedV2(interaction.user, currentCard);
                const [categoryDropdown] = createCategoryDropdown(currentCard);
                ...
                console.log(`📋 Help navigated to card ${currentCard} by ${interaction.user.username}`);
            } catch (error) {
                console.error('Error handling help navigation:', error);
                if (!interaction.replied && !interaction.deferred) {
                    await interaction.reply({ content: '❌ Error navigating help menu', ephemeral: true }).catch(() => {});
                }
            }
            return;
        }
...
client.on('interactionCreate', async interaction => {
    ...
    try {
        ...
        // Handle category dropdown selection - Components v2 UI (fallback handler)
        if (interaction.customId === 'help_category_select') {
            ...
            // Update the message with Components v2 UI
            await interaction.deferUpdate();
            await interaction.editReply({
                embeds: [categoryEmbed],
                components: [categoryDropdown, navRow]
